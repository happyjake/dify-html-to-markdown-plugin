...
class HtmlToMarkdownTool(Tool):
    ...
    def _extract_with_markdownify(self, html_content: str) -> dict:
        ...
        try:
            ...
            for line in lines:
                ...
                if any(js_pattern in line for js_pattern in [
                    'function(', '&&', '||', '===', '!==', 'typeof',
                    'undefined', '.prototype.', 'return', 'var ', 'let ',
                    'const ', '}.bind(', '.call(', '.apply('
                ]):
                    skip_block = True
                    continue
    ...
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        if conversion_method == 'trafilatura':
            ...
            yield self.create_text_message(f"Error: Unknown conversion method '{conversion_method}'")
            return
        
        if not result['success']:
            yield self.create_text_message(f"Error converting HTML to markdown using {conversion_method}: {result['error']}")
            return
        
        if not result['content']:
            yield self.create_text_message(f"Warning: {conversion_method} produced empty output")
            return
